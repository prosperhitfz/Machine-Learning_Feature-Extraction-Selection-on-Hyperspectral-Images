...
def classification_by_mlp(inner_data, inner_label):  # 神经网络分类器（2层hidden layer，分别为100，50个神经元，共迭代一百万次）
    ...
    train_prediction_res = np.reshape(mlp.predict(X_train), (-1, 1))
    test_prediction_res = np.reshape(mlp.predict(X_test), (-1, 1))
    # print(test_prediction_res[1][0])
    # print(np.reshape(y_test, (1, -1)))
    # print(np.reshape(test_prediction_res, (1, -1)))
    print('训练集分类错误率（也称 经验误差）：', 1 - accuracy_score(y_train, train_prediction_res))
    print('测试集分类错误率（也称 错误率、泛化误差）：', 1 - accuracy_score(y_test, test_prediction_res))
    print('精度：', accuracy_score(y_test, test_prediction_res))

    # 计算宏PR值和宏F1 Score（这里的宏意指多分类的PR参数和F1 Score，见周志华西瓜书第二章）
    record_list = np.zeros(y_test.shape)  # 创建一个用于记录每类（标签）有多少个数据的数组
    for i in range(y_test.shape[0]):
        record_list[y_test[i][0]][0] += 1  # 记录每一类有多少数据
    P_mid = 0
    R_mid = 0
        ...
        for j in range(record_list.shape[0]):
            ...
            if record_list[i][0] != 0 and record_list[j][0] != 0 and (i < j):
                ...
                for k in range(test_prediction_res.shape[0]):
                    if y_test[k][0] == i and test_prediction_res[k][0] == i:  # 被正确预测为正类的正类样本数量
                        TP += 1
                    elif y_test[k][0] == i and test_prediction_res[k][0] == j:  # 被错误预测为正类的负类样本数量
                        FN += 1
                    elif y_test[k][0] == j and test_prediction_res[k][0] == i:  # 被错误预测为负类的正类样本数量
                        FP += 1
                    elif y_test[k][0] == j and test_prediction_res[k][0] == j:  # 被正确预测为负类的负类样本数量
                        TN += 1
    ...
    return y_test, test_prediction_res


def visualization_res_matrix(initial_matrix, classify_matrix):  # 将矩阵结果可视化
    initial_img = np.reshape(initial_matrix, (25, -1))  # 将类别矩阵恢复为原来的二维图像尺寸
    classify_img = np.reshape(classify_matrix, (25, -1))  # 将分类矩阵恢复为原来的二维图像尺寸
